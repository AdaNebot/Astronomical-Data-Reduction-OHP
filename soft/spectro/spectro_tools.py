...
def read_raw_spectrum_old(fits_file,get_header=0):
    # This is for reading AURELIE spectra from before 2020 or so
    # returns an index and the fluxes
    # Open the FITS file
    hdulist = fits.open(fits_file)
    header = hdulist[0].header
...
def _gaussian_plus_bg(x, *params):
    # just like the gaussian but with a constant background under it
...
def generate_first_guess(peaks,amp=20000.):
#    npeaks=len(peaks[:3])
    npeaks=len(peaks)
    initial_cen=peaks
    initial_amp=np.full(npeaks,amp)
